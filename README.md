...
1. Запустите Docker Desktop и откройте корневую папку репозитория, в который лежит _Makefile_, в консоли из которой Вы будете работать.

2. В консоли введите команду `make webapi` и перейдите на адрес <http://localhost:5000/health>. Вы должны увидеть сообщение `Ok`, которое означает, что запуск программы произошёл успешно.
